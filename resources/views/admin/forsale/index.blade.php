...
<div class="row align-items-center pb40">
            ...
            <div class="col-xxl-9">
              <div class="dashboard_search_meta d-md-flex align-items-center justify-content-xxl-end">
                ...
                <a href="{{route('admin.forsale.create')}}" class="ud-btn btn-thm">Add New Property<i class="fal fa-arrow-right-long"></i></a>
              </div>
            </div>
          </div>
          <div class="col-lg-6">
                        ...
                        <th scope="row">
                          <div class="listing-style1 dashboard-style d-xxl-flex align-items-center mb-0">
                            ...
                            @foreach($images as $image)
                              <img class="w-100" src="{{ asset('public/forsale/multiimage/'.$image) }}" alt="">
                            @endforeach -->
                            <img class="w-100" src="{{ asset('storage/appforsale/thumbnail/'.$forsale->thumbnail_image) }}" alt="">
                            <!-- <img class="w-100" src="{{ asset('public/forsale/thumbnail_image/'.$forsale->thumbnail_image) }}" alt=""> -->
                            </div>
                            <div class="list-content py-0 p-0 mt-2 mt-xxl-0 ps-xxl-4">
                              <div class="h6 list-title">{{$forsale->title}}</div>
                              <p class="list-text mb-0">{{$forsale->address}}</p>
                              <div class="list-price">${{$forsale->lot_sale_price}}/<span></span></div>
                            </div>
                          </div>
                        </th>
                        <td class="vam">{{$forsale->created_at}}</td>
                        <!-- <td class="vam"><span class="pending-style style1">Pending</span></td> -->
   
                        <td class="vam">
                          <div class="d-flex">
                            <a href="{{ route('admin.forsale.edit', $forsale->id) }}" class="icon" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit"><span class="fas fa-pen fa"></span></a>
                            

                            <form action="{{route('admin.forsale.destroy', $forsale->id)}}" method="POST" class="">
														@method('delete')
														@csrf
