					...
					<table class="sa-datatables-init text-nowrap" data-order="[[ 1, &quot;desc&quot; ]]" data-sa-search-input="#table-search">
						...
						<tbody>
							...
							<tr>
								...
								<!-- <td>
									<a class="text-reset" href="app-order.html">#3201</a>
								</td> -->
								<td>{{$order->id}}</td>
								<td>{{$order->date}}</td>
								<td>{{$order->product->name}}</td>
								<td>
									<a class="text-reset" >{{$order->user->name}}</a>
								</td>
								<td>
									<div class="d-flex fs-6">
										<div class="{{ $order->paid == 'yes' ? 'badge badge-sa-success' : 'badge badge-sa-danger' }}">
											{{$order->paid}}
										</div>
									</div>
								</td>
								<td>
									<div class="d-flex fs-6">
										<div class="{{ $order->status == 'Approved' ? 'badge badge-sa-success' : 'badge badge-sa-danger' }}">
											{{$order->status}}
										</div>
								...
								<td>
									<div class="sa-price">
										<span class="sa-price__symbol">$</span><span class="sa-price__integer">{{$order->order_price}}</span><span class="sa-price__decimal">.00</span>
									</div>
								</td>
								<td>{{$order->item}}</td>
								<td>
									<div class="sa-price">
										<span class="sa-price__symbol">$</span><span class="sa-price__integer">{{$order->total}}</span><span class="sa-price__decimal">.00</span>
									</div>
								</td>
								<td>
									<div class="dropdown">
										...
										<ul aria-labelledby="order-context-menu-0" class="dropdown-menu dropdown-menu-end">
											...
											<li>
												<a class="dropdown-item" href="{{route('admin.order.edit', $order->id)}}">Approved Order</a>
											</li>
											<!-- <li>
