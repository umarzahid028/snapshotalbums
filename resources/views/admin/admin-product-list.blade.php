						...
						<table class="sa-datatables-init" data-order="[[ 1, &quot;asc&quot; ]]" data-sa-search-input="#table-search">
							...
							<tbody>
								...
								<tr>
									...
									<td>
										<div class="d-flex align-items-center">
											...
											<div class="sa-symbol sa-symbol--shape--rounded sa-symbol--size--lg">
												<img  height="40" src="{{asset('storage/appproduct/'.$product->image)}}" alt="{{$product->image}}" width="40"></div></a>
											<div>
												<a class="text-reset">{{$product->name}}</a>
												<div class="sa-meta mt-0">
													<ul class="sa-meta__list">
														<li class="sa-meta__item">ID: <span class="st-copy" title="Click to copy product ID">{{$product->id}}</span></li>
														<li class="sa-meta__item">SKU: <span class="st-copy" title="Click to copy product SKU">{{$product->sku}}</span></li>
													</ul>
												</div>
											</div>
										</div>
									...
									<td>
										<a class="text-reset">{{$product->category->name}}</a>
									</td>
									<td>
									...
									<td>
										<div class="sa-price">
											...
											<span class="sa-price__symbol">$</span><span class="sa-price__integer">{{$product->P_for_retailer}}</span><span class="sa-price__decimal">.00</span>
										...
										<div class="dropdown">
											...
											<ul aria-labelledby="product-context-menu-0" class="dropdown-menu dropdown-menu-end">
												...
												<li>
													...
													<form action="{{route('admin.product.destroy', $product->id)}}" method="POST" class="">
														@method('delete')
														@csrf
